...
踏んでいない規律は、あなたの環境ではまだ雑音だ。規律を積みすぎたエージェント指示ファイルは、守られなくなる——だからこのキットは L1（価値判断モデル）に **本文160行・原則32個**という予算を敷いている（理由は [`docs/faq.md`](../../docs/faq.md) の「なぜ160行で切るのか」）。**借り物の原則は、自分で焼いた原則と同じ枠を食う。** まとめて貼れば、あなた自身の判断が入る場所がその数だけ減る。
...
※ この5点の背後の原理（系列位置効果・信号検出・再認＞想起・デフォルト効果・知識の呪い）と、前提ゼロゲートの検査観点10問は [`../docs/decision-cards.md`](../../docs/decision-cards.md)。ここは現物だけ。
...
### D1. Don't ask what they already answered.

**［単体で効く］**
**貼り先**: `verifiers.md` (B) — 対外文書のDoDに1行足す

相手への質問は、送る前に**1問ずつ過去のやり取りに当てる**。「未解決リストに残っている＝まだ聞いていない」ではない——リストの更新漏れかもしれない。聞くべきを聞かないのと、もらった答えをまた聞くのは、同じ失敗の両面で、どちらも送信前に記録を1回開けば防げる。

> 焼けた出自: 納期直前、多忙な相手への確認メールに4問を載せた。うち**3問は相手がすでに答えていた**（1問は会議の逐語に、1問は先方が送ってきた名簿そのものに、1問はこちらの資料に答えがあった）。未確定リストからの機械転記が原因——リストの鮮度管理と、送信前の検品の両方が抜けていた。

**機械化**: 送信前チェック1問——「この質問リストの各行について、相手の返信・議事録・受領物を検索したか」。

### D2. Every line of an agreement has an origin — their words, their document, or your guess.
...
**機械化**: 文書の書式に出所列（または〔当方案〕札）を持たせ、札なしの行を lint で数える。

### D3. One canonical copy; everything else is a pointer.

**［単体で効く］**
**貼り先**: エージェント指示ファイル・案件フォルダの作法

正本は1か所に1本。作業用のコピーを置いた瞬間、**二重に育ち始める**。持ち歩きたければコピーではなくリンク（ポインタ・ショートカット）を置く。「この条件、いつ決まったっけ？」に改訂履歴で答えられなくなったら、もう分裂している。

> 焼けた出自: 合意文書の正本を成果物側に置いたまま、作業場所にコピーを作って**そちらだけを1か月改訂し続けた**。相手に「この数値いつ入った？」型の確認が要る局面で、履歴が正本側に無く、答えられなかった。リンク化で解消——分裂は起きた後で気づくもので、起きる前に禁じるしかない。

**機械化**: 正本と同名のコピーがリンクでなく実体だったら警告する 1 行チェック。
