...
**エントリは溜まっているのに昇格が0件のときは、まず配送を疑う。** 候補が棚に積まれていることと、その候補が人間の前に運ばれて採否を問われたことは別の工程で、後者が欠けると昇格は0のまま止まる（**設計上あり得る失敗モード**としての説明と直し: [`distillation-loop.md`](distillation-loop.md)）。
