...
| **`templates/skills/advisor-gate/SKILL.md`** | **Advisor gate** — a full procedure for consulting a frontier-class external reasoning model on high-stakes decisions: decision-pivot based context sufficiency (instead of "be MECE"), a standard prompt template, anti-sycophancy rules, and a review/acceptance flow. Copy into your agent's skills directory to use. |
| **`templates/skills/meeting-copilot/`** | **Meeting copilot** — a two-machine live meeting monitor: a Windows laptop streams your mic and a loopback of the other side's voice to a parent machine, which transcribes both, drives a teleprompter you read from, checks every incoming utterance against your facts ledger (contradiction / already-known / new), and answers off-script questions. Ships the parent daemons, the Windows capture agent, and worked config examples. Consumes raw transcripts only — never a meeting-AI summary. **One sheet, not two**: `build_agenda.py` turns the agenda page you hand the other side into both the teleprompter and the step machine, so the two cannot drift apart. Every card carries the same three lines — *what it is about / what happened / the sentence to say* — with a to-whom label, and a repeated nudge collapses into a count instead of firing again. When you start hunting for a URL or an account name mid-meeting, it reads it off your own quick-facts sheet (locations only for anything secret). Each daemon watches one stop file and folds itself up, so nothing is left running after the call. |
...
| **`templates/agent_instructions.md`** | **The instance constitution** — scaffolded as `AGENTS.md` (which Codex reads directly, and Claude Code reads through the one-line `CLAUDE.md` beside it). One text, two names. |
...
| **`templates/skills/advisor-gate/SKILL.md`** | **高品質モデル諮問（advisor-gate）**——重要な判断を最上位の外部推論モデルにレビューさせるための手順一式。「MECEに渡せ」を判断ピボット方式の十分性判定に置き換えたコンテキスト組み立て・標準テンプレート・追従対策・検収の流れ。エージェントのスキルディレクトリへコピーして使う。 |
| **`templates/skills/meeting-copilot/`** | **会議同席モニタ（meeting-copilot）**——2台構成のリアルタイム同席道具。子機（Windowsノート）が「自分のマイク」と「相手の声のループバック」の2系統を親機へ送り、親機が文字起こし・台本のテレプロンプター表示・**事実台帳との突き合わせ**（矛盾／既知／新規）・台本外質問への回答を行う。親機の常駐一式・子機の取り込み一式・設定の実例つき。入力は逐語のみで、議事録AIの要約は使わない。**台本は書かない**——相手に渡す進行表1枚から `build_agenda.py` がカンペと段取りを作るので、二重管理で食い違わない。カードは全部**3行固定**（【対象】【状況】【言うこと】＋宛先ラベル）で、同じ催促は初回だけフルカード・以後は未解決の件数バッジになる。会議中に URL やアカウント名を探し始めると、自分で用意した即答表から読み上げる（秘密は値でなく所在だけ）。終話を検知したら全層が停止ファイルを見て自分で畳む。 |
...
| **`templates/agent_instructions.md`** | **実走環境の憲法**——`AGENTS.md` として生成される（Codex は直読・Claude Code は隣の1行だけの `CLAUDE.md` 経由で取り込む）。本文は1つ・名前が2つ。 |
