...
| **`templates/skills/meeting-copilot/`** | **会議同席モニタ（meeting-copilot）**——2台構成のリアルタイム同席道具。子機（Windowsノート）が「自分のマイク」と「相手の声のループバック」の2系統を親機へ送り、親機が文字起こし・台本のテレプロンプター表示・**事実台帳との突き合わせ**（矛盾／既知／新規）・台本外質問への回答を行う。親機の常駐一式・子機の取り込み一式・設定の実例つき。入力は逐語のみで、議事録AIの要約は使わない。 |
