...
意味の正しさまで機械に判定させないこと。機械で判定できる層（表記 lint・正本突合・日付検算・構造整合・**未確認の断定と否定の射程**）だけを機械に任せ、意味の判定は人間に残す。それでも体感ではエラーの過半が機械層で死ぬ。`verifiers.md` の (A) が機械層、(B) が人間の判断を残した成果物タイプ別 DoD。
...
3つ疑う。(1) 線引きが厳しすぎる（**戻せる操作をキューに積んでいる**——内向き/外向きは近似なので、`docs/design.md` の分類表と「戻せるか」の軸で見直す）。(2) 「迷い」欄が薄くて1件ごとに精読させられている。(3) 生成トリガーの頻度が処理量を超えている（夜間停止・頻度を下げる）。溜まること自体が停止規則のシグナル。
