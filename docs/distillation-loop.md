...
## どのCLIのログを読むか（Claude Code / Codex）

採取の3本（[`correction_scan.py`](../scripts/correction_scan.py)・[`discipline_scan.py`](../scripts/discipline_scan.py)・[`mine_conversations.py`](../scripts/mine_conversations.py)）は、**2つのCLIのログを同じ形に均して**読む。読み分けは [`scripts/lib/log_sources.py`](../scripts/lib/log_sources.py) の1か所にあり、スクリプト側には「どのターンが人間の声か」の判断だけが残っている。

| | 置き場 | 1セッション | サブエージェント |
|---|---|---|---|
| Claude Code | `~/.claude/projects/<スラッグ>/<session-uuid>.jsonl` | 1ファイル | 親と同じファイルの中に `isSidechain` 印で混ざる |
| Codex CLI | `~/.codex/sessions/YYYY/MM/DD/rollout-<ISO>-<uuid>.jsonl` | 1ファイル（日付で階層化） | **別ファイル**。ヘッダが `thread_source: "subagent"` |

```bash
--source claude|codex|auto   # 既定は $LOG_SOURCE、無ければ auto（＝この機械に在る方を全部）
--dir DIR                    # Claude Code のプロジェクトログ。繰り返し可
--codex-dir DIR              # Codex の sessions ルート（既定 $CODEX_SESSIONS_DIR）
--codex-cwd PATH             # そのパス配下で走ったCodexセッションだけ（既定 $CODEX_CWD_FILTER）
```

**`--dir` を名指ししたら Claude Code だけを読む**（`--codex-dir`／`--codex-cwd` を併記しない限り）。auto の「在る方を全部」より、**名指しの方が強い**——ログの場所を指定するのは範囲を切る行為で、そこを勝手に踏み越える既定は親切ではない。

**Codex には「プロジェクトごとのディレクトリ」が無い。** 全案件が1本の木に入るので、`--dir` に相当する絞りは `--codex-cwd`（＝`config.env` の `CODEX_CWD_FILTER`）だ。cron に書き忘れると、その案件の監査に他案件のセッションが混ざる——**エラーにはならず、数字が増えるだけ**なので気づけない。

Codex 側で吸収している落とし穴が3つある。どれも**壊れ方が「静かに数が変わる」**ので、[`tests/unit_codex_logs.py`](../tests/unit_codex_logs.py) で固定してある。

1. **同じターンが2回流れる。** ユーザ発話もアシスタント発話も `response_item` と `event_msg` の両方に出る（新しいビルドでは `item_completed` にも）。両方数えると全部が倍になる。読むのは `response_item` だけ。
2. **ハーネスがユーザの声で喋る。** `<environment_context>` は8件に1件くらいの頻度でユーザメッセージに紛れ、しかも**多くは「人間の実プロンプト＋注入文」が1メッセージ2パート**という形で来る。だから**パート単位で捨てる**——メッセージ単位で捨てると人間のプロンプトごと消える。`developer` ロール（基礎指示）は丸ごと捨てる。
3. **サブエージェントは別ファイル。** ヘッダの `source` が、通常セッションでは文字列なのに**サブエージェントではオブジェクト**になる。ここを文字列前提で読むと落ちる。

### sqlite 移行後の扱い

**2026-09-06 に codex-cli 0.149.0 で実測**した範囲では、rollout JSONL は**現役**だ。`codex exec` を1回走らせたところ、`~/.codex/sessions/2026/09/06/rollout-*.jsonl` が新規に書かれ、本文（ユーザ発話・アシスタント発話）がそのまま入っていた。同時に `~/.codex/thread_history_*.sqlite` も更新され、セッションヘッダの `history_mode` は `"paginated"`（古いログでは `"legacy"` か欠落）になっている。移行コマンド `codex migrate-rollouts` も在る（`--apply` を付けない限り報告のみ）。

**この読み手は JSONL だけを読む。sqlite は未対応**（`thread_items` / `thread_turns` というテーブルが在ることは確認したが、スキーマが公開契約だという根拠が無いので当てにしない）。

〔未確認〕**将来のビルドが JSONL を書かなくなるか**は分からない。そうなったときの症状は「採取が静かに0件になる」なので、判定はこれで足りる——**Codex を使った日に `ls ~/.codex/sessions/$(date +%Y/%m/%d)/` が空なら、JSONL は止まっている。** その日が来たら sqlite 読みの `LogSource` を1つ足す（スクリプト3本は触らずに済む）。
...
> **この節は設計上あり得る失敗モードの記述であって、実測の報告ではない。** 以前この位置には作者の実インスタンスの観測として日数と件数を書いていたが、同じリポジトリが公開している固定証拠 [`evidence-v1.0.0`](https://github.com/yohey-w/kagemusha/tree/evidence-v1.0.0) と矛盾するため撤回した（来歴は [`provenance.md`](provenance.md)）。**残しているのは、部品が全部健康なまま昇格だけが止まりうるという壊れ方の説明だけ**で、その壊れ方が起きた事例をこのキットは主張しない。
