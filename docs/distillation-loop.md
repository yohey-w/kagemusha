...
3. **蒸留（たまに）**: 候補を8欄の審査書式（規律案・型・**原文**・適用範囲・例外・確信度・反例・貼り先・鮮度）で審査キューへ追記するだけ。既存原則と矛盾する候補は解決せず「競合・保留」枠へ回す。
