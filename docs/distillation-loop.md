...
> **実測（n=1・このキットの作者の実インスタンスでの観測。効果量も一般化可能性も主張しない）:** 週次の蒸留は正常に動作し、昇格候補を生成し続けていた。だが候補ファイルが人間の前に運ばれないまま**1週間**放置され、**運用15日間で規約への昇格は0件**だった。故障した部品は1つも無く、欠けていたのは候補をファイルから人間の視界へ運ぶ工程だけだった。**同じ候補群を、ファイルとして置いた1週間は0件処理・1件ずつ問いの形で提示したところ、その場で3件が裁可された。**
