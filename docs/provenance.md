...
| **訂正の昇格——採取は毎日ただで、蒸留は材料が貯まった日だけ**（第3.5章・第8章） | `scripts/correction_scan.py`（毎日の採取・LLM不使用・**言い換えはイベントに束ねてから数える**・subagent発話とツール出力は採らない・訂正語彙は**同梱せず** `--patterns` 必須）＋`scripts/distill.sh`（**材料**トリガー・閾値未満は沈黙・7日フォールバックは1件以上ある時のみ・FIRED/SKIPPED/**FAILED**を潰さない・**キューを読み戻してから**ラチェットを進める）＋`templates/distill-prompt.md`（規律案1行＋8欄の審査書式・仮説を発明しない・既存原則との競合は**解決せず保留枠**・書いてよいのは審査キューだけ）＋`templates/promotion_queue.md`＋`templates/correction_patterns.example.txt`（メニュー）＋`docs/distillation-loop.md`＋`tests/test_g_distill.sh`（群G・86アサーション） | `46cedb8`（2026-08-05） | ［台帳］承認者の一言「**汎用蒸留便は毎日の方がよくないか**」から着工し、**毎日やるのは採取だけ・蒸留は材料で発火**へ設計を割った（材料が薄い日にモデルを焚くと、モデルは「足りない」とは言わず薄い材料から原則をひねり出す）。**別血統（Codex）の検算で設計の穴が5つ出て、着工中に反映**——言い換えの多数決化・既存原則との競合・審査書式の薄さ・失効の欠落・素材の長さ上限。**実装中にテストが自前の穴を1つ掘り当てた**: 「一度も蒸留していない」を「期限超過」と扱ったため、**新規インストールが初日に週次フォールバックで発火し、閾値を無効化していた**（`first_scan` で時計の原点を持たせて修正・回帰テストで固定） |
