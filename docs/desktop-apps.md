...
A standalone request authorizes that one message. When an authorized workflow such as
`codex-pro-plan-build` calls the skill, each invocation still sends one message exactly once, but the
workflow may make useful follow-up consultations within its approved purpose and information scope
without per-message approval. Explicit count, cost, or time limits still apply, and any expansion of
purpose or sent information returns to the user first.
...
asks only when the external purpose, allowed outbound evidence, or Web model/mode is unresolved. It
...
approved purpose and information scope needs no extra approval. Explicit count, cost, and time limits
are honored; expanding the purpose or sent information requires confirmation. The workflow avoids
repeating the same question with the same evidence and stops with a reason when no new question or
evidence can produce progress. If local model/effort proof is found missing later, existing code and
evidence are preserved, the condition is reported as unverified, and the user chooses whether to
relax it or request bounded review or tests from a verified executor; a full rewrite is not automatic.
...
単発依頼が認可するのはその1メッセージです。`codex-pro-plan-build` など認可済みworkflowから呼ぶ場合も、
各呼び出しは1メッセージをexactly-once送りますが、許可済みの目的・情報範囲内の有益な再相談は逐一承認なしで
続けられます。明示された回数・費用・時間制限は守り、目的や送信情報を広げる前には利用者へ戻ります。
...
相談する場合に質問するのは、外部送信の目的、送ってよい証拠、Webモデル/モードのうち未解決な項目だけです。
...
再相談が既認可の目的・情報範囲内なら逐一の追加承認は不要です。明示された回数・費用・時間制限を守り、目的や
