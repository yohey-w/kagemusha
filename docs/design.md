...
| **検証器** | チェックリスト | `templates/verifiers.md`（機械層の lint／正本突合／日付検算／構造整合＋成果物タイプ別 DoD） |
| **停止規則** | 締切 | 監査は最大2周（`verifiers.md`）／**夜間は回さない**／締切レーダーは `tasks.md` から生成 |
| **マンデート** | 決裁 | **内向き＝自動、外向き＝`approval_queue.md`**。何が内向き/外向きかは下表 |

---

## 内向き / 外向きの分類（マンデートの中身）

マンデート設計の実体は、この一枚の分類だけだ。迷ったら外向き扱い（安全側）。
...
| その他 | 読み取り全般 | デプロイ・push・外部 API の書き込み系 |
