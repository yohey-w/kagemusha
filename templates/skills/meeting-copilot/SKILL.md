...
🔴 **判定は本線の外で走ります。** 1発話ごとに「判定器のタイムアウト＋退避先の
タイムアウト」だけ待つ作りにすると、発話が4秒に1本来る会議では本線が二度と
追いつきません。だから班は**1本だけ**走らせ、走っている間に来た発話は判定を
**見送ります**（前提監視と同じ作法。見送りはログに出ます）。起動時に
「1発話あたり最大◯秒」を読み上げ、2秒を超える鎖なら警告します——
**会議では `fallback_chain: [jev, rules]` を勧めます**（小型 LLM の3秒を挟むと、
