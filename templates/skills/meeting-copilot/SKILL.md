...
**退避の順序は `fallback_chain`**（既定 `[jev, llm, rules]`）。前から順に試し、
答えた実装で止まります。**最後は必ず `rules`**——外へ出ない実装で終わらないと、
全部落ちたときに答えが1つも出ません（書き忘れても足します）。使わない段は
外してよく、`[jev, rules]` なら LLM を挟みません。`base_url` が空の段は鎖から
自動で外れます。

`decisions.jsonl` の `backend` 欄に、**実際に答えた実装**が残ります
（`jev` / `llm(fallback)` / `rules(fallback)`）。ここを見ないと、
「判定器で測った」つもりの数字が実はルールの数字、が起きます——実測で
2,077発話のうち 1,754 が退避でした。
...
`--backend llm` で LLM 単独の成績を測れます（鎖を LLM から始める）。Jev と
並べて的中率を見てから、どちらを本線にするか決めてください。
