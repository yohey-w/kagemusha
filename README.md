...
| connectors, unattended | works, with `--allowedTools mcp__…` | works; **no per-tool allowlist exists, and the plugin enable switch does not switch anything off** (measured 2026-09-07) — the outward brake is a `PreToolUse` hook, shipped ([`docs/inbound-loop.md`](docs/inbound-loop.md)) | keep the sweep read-only with a hook, not with a sentence |
