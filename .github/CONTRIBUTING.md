# Contributing — two lanes

*English first, 日本語は下。*

There are two ways in, and they are not the same road with different traffic. One is judged by a maintainer; the other is judged by a string matcher and merged without anybody reading it. Pick the lane before you write the PR.

| | **main lane** (the kit) | **`community/` lane** (your shelf) |
|---|---|---|
| What you change | `templates/starter-disciplines.md`, docs, formats, scripts | `community/<your GitHub login>/*.md` only |
| What qualifies | the physics of working with an AI, burned in a live loop | anything you burned — including disciplines specific to your profession or environment |
| Who decides | **a maintainer rules on it.** No automation merges here | **a format lint.** Pass and it is auto-approved and squash-merged |
| Turnaround | as long as the ruling takes | minutes |
| Review of content | yes | **none — nobody reads it** |
| Responsibility | shared with the maintainer who merged it | **yours** |
| Take-down | normal PR | issue with the removal template, owner-only, automated |

Read [`community/README.md`](../community/README.md) before using the second lane. It is short, and the part about git history being permanent is the part people skip.

---

## The main lane — the kit itself
...
2. **The proposition survives having the profession stripped off.** *「職業を落としても命題が残ること。」* Only the physics of working with an AI — holes anyone delegating work to an agent falls into, whatever their trade. **Working standalone is not a reason to include it**: a discipline that belongs to your profession goes to your own `judgment/judgment_model.md`, or to the `community/` lane here. This is the condition most PRs fail.
...
Format changes, doc fixes, and scripts are the same lane: a maintainer rules. Keep `scripts/test.sh` green — it is the acceptance gate, it runs in CI unchanged, and there is no skip primitive in it.

---

## The `community/` lane — auto-merged
...
- **Every changed path** — including the old name of a rename — is `community/<the PR author's own login>/<name>.md`. The prefix is built from the PR author's login, so touching another person's shelf and touching the kit are the same case.
...
The full rule table, the format for an entry, and the removal procedure live in [`community/README.md`](../community/README.md).
...
入口は2つあり、同じ道の混み具合の違いではありません。片方は**管理者が裁定**し、もう片方は**文字列照合が判定して、誰も読まないままマージ**されます。PR を書く前にレーンを選んでください。

| | **main レーン**（キット本体） | **`community/` レーン**（あなたの棚） |
|---|---|---|
| 変えるもの | `templates/starter-disciplines.md`・docs・書式・スクリプト | `community/<GitHub ログイン名>/*.md` のみ |
| 載る条件 | 実走で焼けた「AIとの協働の物理」 | あなたが焼いたものなら何でも（職業固有・環境固有の規律も可） |
| 判定者 | **管理者の裁定。** 自動マージは一切なし | **形式 lint。** 合格すれば自動承認＋squash マージ |
| 所要 | 裁定にかかるだけ | 数分 |
| 内容のレビュー | あり | **なし——誰も読みません** |
| 責任 | マージした管理者と分担 | **投稿者** |
| 取り下げ | 通常の PR | 削除テンプレの issue（本人のみ・自動） |

2つ目のレーンを使う前に [`community/README.md`](../community/README.md) を読んでください。短いですが、**git 履歴が恒久である**という段が、いちばん飛ばされる段です。
...
2. **職業を落としても命題が残ること。** 載るのは「AIとの協働の物理」だけ——職業も業種も問わず、AIに仕事を任せる人間なら同じ形で踏む穴に限ります。**技術的に単体で効くことは、ここに載せる理由になりません**。職業の規律の置き場所はあなたの `judgment/judgment_model.md` か、この repo なら `community/` レーンです。**落ちる PR の大半はここ。**
...
書式修正・ドキュメント・スクリプトも同じレーン（管理者の裁定）です。`scripts/test.sh` は緑のまま保ってください——これが受け入れゲートで、CI はこれをそのまま実行し、この suite には skip の仕組みがありません。
...
- **変更ファイルが全て**（リネーム前の名前も含む）`community/<PR作者自身のログイン名>/<名前>.md` であること。接頭辞は PR 作者のログイン名から組み立てるので、**他人の棚を触ること**と**キット本体を触ること**は同じ扱いになります。
...
ルール表の全文・エントリの書式・削除手順は [`community/README.md`](../community/README.md) にあります。
