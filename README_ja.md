...
**仕事は夜のうちに進む。あなたは朝、数分承認するだけ。却下した理由まで、資産になる。**
...
> **ループのある朝。** 夜のあいだにループが回った。06:53、スマホが鳴る（[ntfy](https://ntfy.sh/)）: 今日の盤面——完成した下書き3本・調査メモ1本・承認キューに2件。
...
#    手動なら毎朝のカレンダー通知でも可——スケジューラは何でも等価）。
...
| **マンデート** | **どこまで任せるか・誰が責任を持つか** | **内向き＝自動 / 外向き＝承認キュー** |
