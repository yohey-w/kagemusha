...
kagemusha は、AIエージェントの**「内向きは自動／外向きは人間の承認」**と、却下理由を恒久規律へ戻すループを、**Markdown の書式とスクリプトで配るキット**です。常駐エージェントでも SaaS でもなく、**あなたの判断基準は同梱しません**。
...
移動しました → [`docs/getting-started.md`](docs/getting-started.md#手順コピペ)（前提表・コピペ手順・任意の血統違い検算器）。

## 9. 発展編

移動しました → [`docs/operations.md`](docs/operations.md)（毎日と毎週・複数案件・自分の仕事の数え方 G/S/D/V/I/R）・[`docs/inbound-loop.md`](docs/inbound-loop.md)（世界からの入力を捕まえる）・[`docs/faq.md`](docs/faq.md)（設計の考え方）。
