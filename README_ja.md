...
**配線する前に、動くところを見たい人へ。** `./scripts/demo-distillation.sh` が、訂正 → 採取 → 蒸留 → 審査 → 昇格 → 監査の一周を10分で回して見せる。**API課金ゼロ・あなたのログは読まない・一時サンドボックスの外へは1バイトも書かない**（終了時に消える）→[何が見えるか](#3-クイックスタート30分コピペで動く)。
