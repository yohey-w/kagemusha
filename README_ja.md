...
**却下・修正の理由は、あなたが生む最も価値の高いログ**だ（[§3](#3-これは何を解くのか1分版)）——そして蒸留先は3つある:
...
- **`ssot/norms/<ドメイン>.md` へ**——却下が**成果物への同じ型の直し**（「要約より先に現物を置け」「圧縮した標語より平易な言い方を先に」）なら、**次の初稿を頼むときの指示文に入る規約**へ蒸留する。検査する側に足しても同じ失敗を見つけるのが速くなるだけで、生成側に戻すと最初から起きなくなる——線形と複利の差。→ [`docs/norms-loop.md`](docs/norms-loop.md)

**訂正**が**裁定**より重い理由は [§9](#設計の考え方faq形式) の設計 Q&A。**この一周が実際に閉じた証拠——無人発火した蒸留の実ログと、このリポジトリの原則1本の裏にある台帳エントリ:** [`cookbook/author/evidence/`](cookbook/author/evidence/README.md)。
