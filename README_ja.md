...
- **該当する**: 「この相手にこのトーンは押しつけがましい」という却下が、実発話のまま台帳に残り、審査キューに規律案として差し出され、**あなたがそれを自分の指示ファイルへ写す**。写した行為が昇格だ。
- **該当しない**: 訂正を採取して素材ファイルや台帳へ**積むところまで**。それは事実の保存であって昇格ではない——「**移動には関門は要らない。昇格には要る**」（[`docs/distillation-loop.md`](docs/distillation-loop.md)）。その関門は人間で、通っていないものはまだ昇格していない。

**人間定置網**——**AIの最後に人間を置いたまま、そこで生じた判断を次回のAIへ戻さず、人間が同じ確認を繰り返す運用。**

- **該当する**: 「同じ判断を手で下し続ける——このトーンは却下・その数字を直せ・出典が無いなら書くな。その判断は資産なのに、『却下』を押すたびに捨てられている」（[§3](#3-これは何を解くのか1分版)）。人間は毎回きちんと働いているのに、来週も同じ却下が来る。
- **該当しない**: 不可逆な外向き操作の前に人間を置くこと**そのもの**。それは[依存校正](#依存校正--キュー全体の底にある原則)——戻せない操作には実行*前*の独立検証を要求する、という設計だ。人間がいることが定置網なのではない。**そこで出た判断が次回のAIに戻っていないこと**が定置網。
...
| 工程 | このキットの道具 |
|---|---|
| 採取——訂正を拾い、イベントに束ねる（LLM不使用＝ただ） | [`scripts/correction_scan.py`](scripts/correction_scan.py) |
| 発火——時刻ではなく**材料**が貯まった時だけ焚く | [`scripts/distill.sh`](scripts/distill.sh) |
| 差し出し——規律案1行＋8欄の審査書式で、人間の前に置く | [`templates/distill-prompt.md`](templates/distill-prompt.md) → [`templates/promotion_queue.md`](templates/promotion_queue.md) |
| 事実の保存——追記専用・実発話 quote 付き（**昇格ではない**） | [`templates/decisions_journal.md`](templates/decisions_journal.md) |
| 人間の関門そのもの——外向き操作が止まる場所 | [`templates/approval_queue.md`](templates/approval_queue.md) |
| 昇格先①——却下が**機械的**な穴なら検証器へ | [`templates/verifiers.md`](templates/verifiers.md) |
| 昇格先②——却下が**判断**なら価値判断モデルへ（≤160行） | [`templates/judgment_model.md`](templates/judgment_model.md) |
| 昇格した後——その規律が自分の環境で効いているかの監査 | [`scripts/discipline_scan.py`](scripts/discipline_scan.py) |

全機構は [`docs/judgment-distillation.md`](docs/judgment-distillation.md)、軽い日次レーンは [`docs/distillation-loop.md`](docs/distillation-loop.md)、昇格した後の話は [`docs/discipline-audit.md`](docs/discipline-audit.md)。
...
**あなたが作る実データが git に入らない仕組み**（allowlist 方式の `.gitignore`）は、実際の置き場所と一緒に §9 の「複数案件を回す」に図で示した。
...
§5 は「あなたの却下があなたの資産になる」話だった。同じ輪が、一段上——キット自身にも回っている。
...
なぜ「育つ」が約束ではなく構造なのか: **追記型は積み上がり、スナップショットは腐る。** 凍結されたベストプラクティス集はスナップショットで、あなたの仕事が動いた瞬間に古くなる——§5 が却下を「書き換え」ではなく**追記型の台帳**へ送るのと同じ理由だ。作者がこの題材で書いている長文も同じ継ぎ目で割れている——**理論部は改訂型**（鮮度の日付が要る）、**実践部は追記型**。

**この工程の名前は §5 で定義した——[訂正の昇格](#訂正の昇格)。** 訂正を捕捉し、人間のレビューを通し、正本に定着させる道具は**すでに既製品としてある**（星の付いた OSS にも、製品の標準機能にも）。だからこのキットが配るのはその配管ではなく、**審査の基準の作り方と、昇格した規律の統治**のほうだ——訂正は**事実**なので追記型の台帳に積み、規律は**規範**なので既存の規則を**上書き**する——保存の仕方からして違う。移動や反映に関門は要らないが、**昇格には要る**——その関門は人間だ。
