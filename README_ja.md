...
**同じ送信が、一段下に隠れる——だから歯止めは「許可リスト」。** 実測 2026-09-13。デスクトップ版がモデルに見せる道具は `exec`・`spawn_agent`・`wait_agent` の3つだけで、コネクタは `exec` の中の JavaScript から呼ばれる。ゆえに `tool_name` は `exec` になり、ツール名の動詞を見るフックには送信が一度も見えない。CLI も同じ形だ（83本のロールアウトでコードを実行する 9,222回すべてが `exec`）。そこでガードは `exec` が運ぶコードも読む——ただし**手口を並べるのではなく許可リストで**。許される形は直書きの `tools.<読み取りツール>(…)` ただ一つで、`tools`／`ALL_TOOLS` へのそれ以外の接触はすべて拒否する。手口を数え上げた最初の版は、`const t = tools` で別名を作って添字で引くだけの一行に破られ、さらに11通りで破られた——名前を隠す方法は無限だが、オブジェクトに手を伸ばす方法は有限だからだ。**代償は実在し、承知のうえで払っている**: `exec` の中からツール一覧を取ることはできず、間接的な読み取りもできない。一覧は `AGENTS.md` かスキルに書き、読み取りは直接呼び出しで書くこと。**検出できない形**: このガードが知らない名前で出荷されたコード実行ツール（今日の実測では `exec` ただ一つ）と、`spawn_agent` で起きた子の呼び出しにこのフックが発火するか（**未確認**——実機の Codex でしか答えが出ない）。許可票による送信が `exec` の中でできないのは設計どおり——許可票は引数へ束縛する仕組みで、コード文字列に束縛できる引数は無い。
