...
**入り口は2つあり、順番はこうだ。** ① **まず見るだけ**——10分のデモ。設定も課金も要らず、あなたのファイルには何も書かない。② **自分の環境に入れる**——30分のセットアップ（コピペ）。

**① まず見るだけ（10分）。** コマンド1本で、この道具が何をするものか——あなたがAIに出したダメ出し（「この相手にこのトーンは押しつけがましい」）が、どうやってAIが以後ずっと守る恒久ルールに変わるのか——を、作り物のデータで最後まで実演する。AIは一度も呼ばないので課金は発生せず、あなたのファイルは読みも書きもしない（作業は全部、終了時に消える一時フォルダの中で起きる）。

```bash
git clone https://github.com/yohey-w/kagemusha && cd kagemusha && ./scripts/demo-distillation.sh
```

画面に何が出るかは[まず見るだけ（10分）](#まず見るだけ10分)。

**② 自分の環境に入れる（30分）。** その clone の中でもう1回コピペすると、ファイル一式が置かれる。あとはそのフォルダを AI アシスタント（Claude Code / Codex / 何でも）で開いて、そこで仕事をするだけだ。

```bash
./scripts/setup.sh
```

手順の全文は[自分の環境に入れる（30分・コピペ）](#3-自分の環境に入れる30分コピペ)。
...
- **該当する**: 「同じ判断を手で下し続ける——このトーンは却下・その数字を直せ・出典が無いなら書くな。その判断は資産なのに、『却下』を押すたびに捨てられている」（[§1](#1-これは何を解くのか1分版)）。人間は毎回きちんと働いているのに、来週も同じ却下が来る。
- **該当しない**: 不可逆な外向き操作の前に人間を置くこと**そのもの**。それは[依存校正](#8-依存校正--キュー全体の底にある原則)——戻せない操作には実行*前*の独立検証を要求する、という設計だ。人間がいることが定置網なのではない。**そこで出た判断が次回のAIに戻っていないこと**が定置網。

**判断ループ**——上位概念。**[承認ループ](#1-これは何を解くのか1分版)（生成→検証→内向きは自動／外向きはキューへ→人間が判断）と[判断蒸留](#6-却下を資産に変える--判断蒸留)（却下理由→台帳→価値判断モデル→次セッションのAI）が、1本に閉じた輪。** 新しい機構ではなく、この2つが繋がった状態の名前だ。
...
設定は要らない。課金もない。あなたのファイルには触れない。

```bash
./scripts/demo-distillation.sh          # DEMO_FAST=1 で止まらずに走る・DEMO_KEEP=1 で中身を残す
```

作り物のデータで、一周がそのまま目の前で回る。順番はこうだ:
...
**この工程の名前は冒頭で定義した——[訂正の昇格](#訂正の昇格)。** 訂正を捕捉し、人間のレビューを通し、正本に定着させる道具は**すでに既製品としてある**（星の付いた OSS にも、製品の標準機能にも）。だからこのキットが配るのはその配管ではなく、**審査の基準の作り方と、昇格した規律の統治**のほうだ——訂正は**事実**なので追記型の台帳に積み、規律は**規範**なので既存の規則を**上書き**する——保存の仕方からして違う。移動や反映に関門は要らないが、**昇格には要る**——その関門は人間だ。
